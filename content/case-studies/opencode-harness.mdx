...
metaDescription: "Engineering case study: building a deterministic AI development interface as a VS Code extension. Eliminated the 'Silent Staleness' anti-pattern with active re-derivation, hardened state contracts, and 6,000+ automated tests across unit, contract, and visual layers."
results:
  - value: "5,070"
    label: "Downloads on Open VSX"
    context: "Free installs of the beta client · MIT licensed"
  - value: "6,000+"
    label: "Automated Tests"
    context: "Unit, integration, contract & visual layers"
  - value: "121"
    label: "Contract-Verified Surfaces"
    context: "50 commands · 20 keybindings · 51 settings"
  - value: "18"
    label: "Visual Regression Gates"
    context: "Playwright snapshot suites guarding the webview"
---

<ResultsHighlight results={[
  { value: "5,070", label: "Downloads on Open VSX", context: "Free installs of the beta client · MIT licensed" },
  { value: "6,000+", label: "Automated Tests", context: "Unit, integration, contract & visual layers" },
  { value: "121", label: "Contract-Verified Surfaces", context: "50 commands · 20 keybindings · 51 settings" },
  { value: "18", label: "Visual Regression Gates", context: "Playwright snapshot suites guarding the webview" }
]} />
...
<CaseStudySummary
  ...
  impact="Shipped a free, open-source VS Code client with multiple concurrent AI sessions, 75+ models, and a UI I can actually trust — backed by 6,000+ automated tests. Silent Staleness went from an invisible production risk to a loud, repeatable test failure."
/>
...
> **Open source** — read the code on [GitHub](https://github.com/K-Arthur/opencode-harness) or install it from the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=koarthur.opencode-harness) or [Open VSX](https://open-vsx.org/extension/koarthur/opencode-harness). Everything described below is in the public repo.

> **Current stats (v0.4.70)** — 5,070 downloads on Open VSX, 148 installs with a 5.0★ rating on the VS Code Marketplace, and 11 GitHub stars across a repo with 9 open issues and an active master branch (MIT licensed). The extension tracks the opencode SDK closely, so these numbers move fast.
...
- **It's stateful and long-lived.** Sessions run concurrently — by default 5 simultaneous streams across up to 20 open tabs, with up to 50 sessions stored and restored after a reload. Each has its own model and history. State has to persist across reloads, not reset to a hopeful default.
...
The backend follows a deliberate **thin-orchestrator pattern**: `ChatProvider` delegates to focused services (`TabManager`, `StreamCoordinator`, `MessageRouter`, `SessionDiffHandler`) rather than accumulating logic. That separation matters for the bug hunt ahead — when state is owned by many small services and rendered across a process boundary, it's dangerously easy for one side to hold a value the other side has already moved past.
...
![Inside the editor: the OpenCode panel running a coding session next to the StreamCoordinator source that renders each chunk from live stream state](/images/case-studies/opencode-harness/workflow-ide.png)
*The real thing, in context: the agentic loop on the left, the code that streams it on the right. Each tool call, edit, and result is a projection of live state — not a one-time cached render.*
...
Every command, keybinding, and setting an extension exposes is declared in `package.json`. A surface declared but not wired (or wired but not declared) is a dead wire by definition. Contract tests assert the manifest and the implementation agree across all **121 declared surfaces**:

- **50 commands** — each declared command resolves to a real handler.
- **20 keybindings** — each binding points to a registered command.
- **51 settings** — each setting is read by the code that claims to honor it.
...
Eighteen Playwright snapshot suites photograph the webview in known states and fail the build on unexpected pixel drift — 170+ visual assertions directly targeting the features Silent Staleness attacked:
...
![A real tool-execution session in the OpenCode panel beside the renderer that re-derives each tool card from its live call record](/images/case-studies/opencode-harness/tools-ide.png)
*Exactly the kind of UI state the visual snapshots pin: tool cards, statuses, and outputs that must re-render correctly from live state every time.*
...
| **Test coverage** | 6,000+ automated tests | Unit, integration, contract, round-trip, and 18 visual regression suites |
| **Contract surface** | 121 verified surfaces | 50 commands · 20 keybindings · 51 settings asserted against the manifest |
| **Concurrency** | 5 streams · 20 tabs · 50 sessions | Independent tabs, each with its own model, mode, and persisted history |
| **Reach** | 75+ models | Per-tab selection across Claude, GPT, Gemini, and dozens more |
| **Adoption** | 5,000+ downloads | 5,070 on Open VSX, 148 installs (5.0★) on the VS Code Marketplace, 11 GitHub stars |
| **Accessibility** | WCAG-aligned | 24×24px touch targets (WCAG 2.5.5), high-contrast presets, full keyboard nav, focus traps, `prefers-reduced-motion` |
