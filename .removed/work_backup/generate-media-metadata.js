...
const hasCloudinaryPool = process.env.CLOUDINARY_CLOUD_NAME && process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET;

if (hasCloudinaryPool) {
  cloudinary.config({
  ...
  console.warn('Warning: CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, or CLOUDINARY_API_SECRET is missing. Media metadata generation will be skipped.');
}
...
const main = async () => {
  ...
  if (!hasCloudinaryPool) {
    console.log('Skipping data write as Cloudinary configuration is incomplete.');
    return;
  }
  ...
  if (Object.keys(allMediaData).length === 0) {
    console.warn('\nNo project data was successfully collected. Skipping media metadata file update to prevent data loss.');
    return;
  }
