  ...
  return (
    <div className="space-y-8">
      {posts.map(({ id, title, summary, publishedAt }, index) => (
        <motion.div
          key={id}
          className="group bg-card/80 border border-border/50 rounded-2xl p-6 md:p-8 backdrop-blur-sm transition-all duration-300 hover:bg-card hover:border-border hover:shadow-lg hover-scale"
